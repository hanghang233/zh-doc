...
- CommonJS规范加载模块是同步的，只有加载完成，才能执行后面的操作；AMD规范是非同步加载模块，允许指定回调函数；CommonJS主要用于服务器编程，模块文件一般存在于本地硬盘，所以加载起来比较块。但是如果是浏览器环境，要从服务器端加载模块，这时就必须采用非同步模式，因此浏览器一般采用AMD规范

- 模块化的好处

避免命名冲突、更好的分离，按需加载、更高可复用性、更高可维护性
...
set：集合，由一组无序且唯一不重复的项组成；类似于数组，但是成员的值是唯一的，没有重复值；可用于数组去重

map：类似对象，也是键值对的集合，但是“键”不限于字符串
...
es6块级作用域：内层和外层不受影响，以{}区分

## &55.原型链的作用 ##
- 实现继承：如果没有原型链，每个对象都是孤立的，对象间就没有关联，所以原型链就像一颗树干，从而可以实现面对对象中的继承

- 属性查找：首先在当前实例对象上查找，要是没有找到，那么沿着__proto__往上找

- 实例类型判断：判断这个实例是否属于某类对象

## &56.class和function的区别 ##
class是一个语法糖，本质还是基于原型链；对于function可以用 call apply bind 的方式来改变他的执行上下文，但是class 却不可以，class 虽然本质上也是一个函数，但在转成es5（babel）做了一层代理，来禁止这种行为
- class中的方法不可以用Object.keys来遍历
- function声明是hosting（状态提升）的，而class不是，class需要先声明再使用
- 

## &57. 浅谈js声明提前 ##
js中存在一种机制，在程序正式执行之前，会将var声明的变量和function声明的函数预读到当前作用域的顶；

## &58. == 和=== 和 Object.is的区别  ##
==，等同，两边值类型不同的时候，进行类型转换之后再比较；===，不做类型转换，类型不同一定不相同

===判断是否相等规则：
1. 如果类型不同，就不相等；

2. 如果两个数值，并且是同一个值，那么相等；如果其中一个至少是NaN，则不相等

3. 如果两个是字符串，判断字符串位置

4. 如果两个值都引用同一个对象或函数

5. 如果两个值都是null，或者都是undefined

==判断规则
1. 如果两个值类型相同，详细看===比较规则

2. 如果一个是null，一个是怒的fined，那么相等； null == undefined true

3. 如果一个是字符串，一个是数值，把字符串转换成数值再比较

4. 如果任一值是true，转为1再比较

5. 如果是一个对象，另一个是数值或字符串，把对象转换成基础类型的值再比较

==Object.is判断规则：和===基本相同，但是有两处不同
...
null是一个表示“无”的对象，转为数值为0；undefined表示一个“无”的数值，转为数值为NaN；

当声明的变量还未初始化时，变量的默认值为undefined；null用来表示尚未存在的对象；undefined表示“缺少值”，就是此处应该有一个值，但是没有定义

1+undefined为NaN；1+null为1